...
def train_one_epoch(model, dataloader, opt, params):
    ...
    # #Debugging MAP
    # print([len(val["boxes"]) for x, val in gts.items()])
    # print([len(val["boxes"]) for x, val in dets.items()])
    # first_det = dets[0]["boxes"]
    # first_gt = gts[0]["boxes"]
    # if len(first_det) < len(first_gt) + 5:
    #     print(first_gt)
    #     print(first_det)
    ...
    # Sometimes its nice to comment out training to test the pipeline. This is for that case
    if gts:
...
def fit(params):
    ...
    for epoch in range(params["epochs"]):
        ...
        print(f"EPOCH {epoch} valid loss: {valid_av_loss:.8f} | valid MAP: {valid_MAP:.3f} | train loss: "
