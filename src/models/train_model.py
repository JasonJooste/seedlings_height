...
def get_device(params):
    """ Get the device for a given parameter setting.

    As the devices used either had 1 or 2 GPUs this was enough, but should be rewritten for devices with an arbitrary
    number of GPUs"""
    if "device" in params:
...
def fit_and_test(params):
    ...
    try:
        for epoch in range(params["epochs"]):
            ...
            #### Logging
            metrics = {}
            # Note This could be improved. The metric is overwritten each time
            # Log new weights
            ...
            if valid_MAP > best_MAP:
                # Note: This would be much better with the state dict
